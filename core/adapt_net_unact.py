...
class AdaptNet(nn.Module):
    ...
    def forward(self, data):
        ...
        # print("z",Z.shape)
        # print("zu",ZU.shape)
        del_A_matrix = self.construct_del_matrix_()
        del_z_pred = torch.matmul(Z, torch.transpose(del_A_matrix, 0, 1))
        return del_z_pred
    ...
    def construct_del_matrix_(self):
        ...
        return self.del_A_matrix

    def loss(self, outputs, labels):
        alpha = self.adapt_net_params['l1_reg']
        criterion = nn.MSELoss()
        loss = criterion(outputs, labels) + alpha * torch.norm(self.del_A_matrix, p=1)
        return loss


    def model_pipeline(self, Z, del_Z, print_epoch = True):
