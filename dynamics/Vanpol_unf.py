...
import numpy as np
def dynamics_vanpol_unf(dt, num_traj, num_snaps,num_states, dyn_pars):
    ...
    for i in range(num_traj):
        ...
        for j in range(num_snaps):
            ...
            x2_dot_cor = -a*X_cor[i,j,0] + b*X_cor[i,j,1] -d*(X_cor[i,j,1]**2)*X_cor[i,j,1]
            x2_dot_incor = -(a+amp_a)*X_incor[i,j,0] + (b+amp_b)*X_incor[i,j,1] -(d+amp_d)*(X_incor[i,j,1]**2)*X_incor[i,j,1]
