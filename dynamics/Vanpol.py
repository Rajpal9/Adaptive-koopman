...
def dynamics_vanpol(dt, num_traj, num_snaps,num_states, num_inputs, dyn_pars, Umax):
    ...
    U = Umax*(2*np.random.rand(num_traj, num_snaps)-1)
    U_filtered = np.empty((num_traj, num_snaps))

    for k in range(num_traj):
        U_filtered[k,:] = signal.savgol_filter(U[k,:].T, 15, 3)

    for i in range(num_traj):
        # Initialize matrix for each trajectory
        
        # actual dynamics
        X_cor[i,0,0] = 0.4*(2*np.random.rand(1) - 1)    # E [-1,1]
        X_cor[i,0,1] = 0.3*(2*np.random.rand(1) - 1) #E[-0.inaccurate_model,0.inaccurate_model]
        ...
        for j in range(num_snaps):
            ...
            # U[i,j,:] = Umax*(2*np.random.rand(1) - 1)
            
            # dynamics update
            x2_dot_cor = -a*X_cor[i,j,0] + b*X_cor[i,j,1] - c*U_filtered[i,j] -d*(X_cor[i,j,0]**2)*X_cor[i,j,1]
            x2_dot_incor = -(a+amp_a)*X_incor[i,j,0] + (b+amp_b)*X_incor[i,j,1] - (c+amp_c)*U_filtered[i,j] -(d+amp_d)*(X_incor[i,j,0]**2)*X_incor[i,j,1]
    ...
    return X_cor, X_incor, U_filtered.reshape((num_traj,num_snaps,1))