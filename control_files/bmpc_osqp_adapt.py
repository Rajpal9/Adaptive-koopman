...
class bilinearMPCController():
    ...
    def update_initial_guess_(self):
        ...
        z_last = self.cur_z[-1, :]
        u_new = self.cur_u[-1, :]
        z_new = self.dynamics_object.eval_dot(z_last, u_new, self.z_N0)
        ...
        dz_new = self.dynamics_object.eval_dot(dz_last, du_new, dz_last)
    ...
    def update_linearization_(self):
        ...
        for z, z_next, u in zip(self.z_init[:-1, :], self.z_init[1:, :], self.u_init):
            a, b, r = self.dynamics_object.get_linearization(z, z_next, u, self.z_init[0, :])
            A_lst.append(a)
            B_lst.append(b)
            r_lst.append(r)
