...
class KoopmanNetCtrl(KoopmanNet):
    def __init__(self, net_params,standardizer_x=None, standardizer_u=None):
        super(KoopmanNetCtrl, self).__init__(net_params,standardizer_x=standardizer_x)
    ...
    def process(self, data_x, downsample_rate = 1, train_mode =True):
        n = self.net_params['state_dim']
...
class KoopDNN():
  ...
  def set_datasets(self, x_train, x_val=None):
  ...
  def model_pipeline(self, net_params, print_epoch = True,early_stop=False, plot_data = False):
    ...
    X_train, y_train = self.net.process(self.x_train)
    X_val, y_val = self.net.process(self.x_val, train_mode=False)
  ...
  def test_loss(self, x_test):

    X_test, y_test = self.net.process(x_test, train_mode = False)
